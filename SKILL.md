...
`obsidian://open?vault=<目录名>&file=HelpMeRead/papers/<slug>/course/01-<topic>` 直接自动打开第一节。打开前不要询问用户确认，直接执行。
...
所有分支汇聚后：
- **展示问题答案**：基于 `## 出处` 原文引用或 `## 🔍 深入理解` 提炼当前问题的参考答案（非概念的完整定义），标注 `[原文·已验证]`/`[推断]` 锚点。展示后**直接**润色写入 `## 定义`，不询问确认
- **回写补充层**：同步追加主笔记 `## 学习补充`
- **自动推进**：更新进度后直接告知"进入下一概念：[概念名]"，引导用户继续

用户随时可以说"跳过此概念"快速跳过。完成后可选更新 MOC 状态为 `atomized`。
