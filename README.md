...
- 每节内置：学习目标、核心讲解（含类比 + 公式推导 + **论文原图**）、可选的代码实现、自测小问题（答案折叠，先思考再看）、精选延伸资源链接
...
- agent 自动建脚手架文件（frontmatter / 出处 / 相关都填好），**你只写"用自己的话说这东西是什么"**一句
- 聊天逐个引导，进度可见
- 跨论文去重：同一核心概念在多篇出现时追加出处，不建立重复文件
- 知识真正内化，不代劳也不让你自己折腾文件管理
...
├── references/
│   ├── obsidian-note-template.md                # 五类笔记模板（含脚手架、待学、学习补充）
│   ├── course-design-guide.md                   # 三类课程设计指南（含原图、延伸资源）
│   ├── qa-standards.md                          # 问答标准（五级验证标签 + 引用回查）
│   └── frontmatter-schema.md                    # 全字段 frontmatter schema + MOC
├── examples/                                    # 完整样例（Transformer 论文）
├── README.md
├── README_EN.md
├── LICENSE
└── .gitignore
