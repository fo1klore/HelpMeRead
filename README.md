...
| 日期 | 变更 | 📝 开发者寄语 |
|---|---|---|
| 2026-07-01 | **v2.6** · 课程节角色系统 + 教学深度最低结构：course-design-guide.md 新增 Core/Supporting 节角色定义（Core ≥ max(ceil(总节数 × 50%), 1））和教学深度最低结构（问题动机→变量准备→分步推导→直觉解释→数值例子→错误直觉）；SKILL.md step 2.6 新增节角色检查和 Core 节深度检查（推导分步/前置桥接/数值示例/错误直觉 4 条验证） | 你怎么这样？ |
