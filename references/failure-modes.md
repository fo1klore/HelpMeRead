...
**现象（A）**：步骤 1 类型判定后，agent 直接进入步骤 2 并行生成，未展示文献速览 callout，用户无机会判断"这篇是否值得学"就进入了完整流程。

**原因（A）**：
1. agent 省略了「文献速览」环节（SKILL.md 未强调其为必经步骤）
2. agent 默认用户选择"继续完整流程"，跳过选择环节

**现象（B）**：文献速览展示后，agent 没有默认直接进入步骤 2，而是询问"是否生成笔记"或"要完整学习还是只要笔记"等简化二分问题，扭曲了"默认课程+笔记一体生成"的语意。

**原因（B）**：
1. SKILL.md 歧义词路由指令措辞偏向二分选择（旧版"要完整学习还是只要笔记摘要？"）
2. 文献速览段落将四个选项并列展示，未强调 ① 为默认
3. agent 未遵守"用户不问就不拆"的约束

**修复**（A + B）：
1. 确认 SKILL.md 步骤 1 末尾包含「文献速览」段落
2. 确认速览包含三项：核心贡献、问题-方法-局限、核心概念概览
3. 确认文献速览的默认行为指令明确要求"默认直接进入步骤 2 生成课程+笔记，不询问确认"
4. 确认歧义词路由指令仅按完整流程处理，不在二分语境中确认
5. 确认有一条"绝对不要主动问是否生成笔记或只要笔记还是课程"的强制指令
6. 用户明确说出"只要总结"/"只要笔记"/"只要课程"/"停止"等关键词时才停下确认

**检测命令**：
```bash
# 检查 SKILL.md 中是否存在文献速览段落
grep -q "文献速览" ../SKILL.md && echo "✅ 已包含" || echo "❌ 缺失"
# 检查默认行为是否为直接进入步骤 2
grep -q "默认直接进入步骤 2" ../SKILL.md && echo "✅ 默认行为正确" || echo "❌ 默认行为缺失"
# 检查歧义词路由是否不再用二分措辞
grep -q "要完整学习还是只要笔记摘要" ../SKILL.md && echo "❌ 旧措辞残留" || echo "✅ 旧措辞已清除"
```
