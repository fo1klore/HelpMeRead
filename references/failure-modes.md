...
2. 确认概念热身（Warm-up）模块存在，且其中列出了本节所有核心概念并附通俗解释
...
**现象**：对概念 A 的解释句本身包含概念 B 的术语，而 B 在前文和概念热身中均未解释。例如"Transformer 使用 self-attention 机制"——self-attention 是另一个需要解释的概念。

**原因**：agent 在写解释句时使用了论文语言而非日常语言，混淆了"领域内公认术语"和"通用常识"。

**修复**：
1. 检查每个 `[[概念名]]` 首次出现时的解释句是否包含另一个 `[[...]]` 标记或候选概念名
2. 如果包含 → 拆分：要么在本次热身中先解释被嵌套的概念，要么改写解释句用日常语言绕过

**检测命令**：
```bash
# 扫描课程文件中概念热身模块内的解释句是否包含其他 `[[概念名]]`
