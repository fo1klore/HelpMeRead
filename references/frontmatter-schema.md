...
本文件定义六类产物：文献总结笔记/原子笔记/待学习笔记/问答记录/课程文件/MOC 的 frontmatter 全字段，以及配置文件 `~/.help-me-read.json`。让 Obsidian 的 Bases / graph / 搜索能力真正可用。
...
**去重机制**：
- `defined_in` 是列表，多篇论文标了同一核心概念时，**追加到此列表**，不新建文件
...
| [[HMR-bert]] | research | unread | nlp |

（用 Obsidian Bases 生成动态表格视图，基于 frontmatter 字段 `status` `type` `area` `venue` 等）
